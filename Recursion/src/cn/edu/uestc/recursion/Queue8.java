 ...
 *      被static修饰的成员变量或成员方法独立于该类的任何对象（实例），被所有类对象共享。
 *      如果一个实例对象对静态成员变量做了修改，其他实例对象访问该静态成员变量也会受到影响。
 */
public class Queue8 {
    int max = 8;
     ...
     *
     *      编程的思路： 1、明确输入的n代表第几个皇后，这个n会在放好当前皇后（判断不冲突）之后+1，进行递归
     *                  2、完成每一行n的操作后，开始进行每一列的操作，即来一个循环，从0-7列判断能不能放皇后。
     *                  3、续2，如果冲突了，就for循环下一轮判断，如果这一轮都冲突，这一次递归执行完毕，推出，进入上一轮递归，接着找下一列。
     *                  4、类似的，递归的退出条件一定要写！
     *
     * @param n 第n个皇后
     */
    private void check(int n){
        if (n == max){//这条语句相当于是退出条件，一直在递归n+1
